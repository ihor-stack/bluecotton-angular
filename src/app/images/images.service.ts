...
export class ImagesService {
  ...
  saveImage(image: Image): Observable<Response> {
    ...
    if (image.height > maxHeight || image.width > maxWidth) {
      image = this.resizeImage(image, maxHeight, maxWidth);
    }
  ...
  private resizeImage (image: Image, maxHeight: number, maxWidth: number) {
    if (image.height > maxHeight) {
      const ratio = (image.height - maxHeight) / image.height;
      image.height = maxHeight;
      image.width = Math.floor(image.width * (1 - ratio));
    }
    
    if (image.width > maxWidth) {
      const ratio = (image.width - maxWidth) / image.width;
      image.width = maxWidth;
      image.height  = Math.floor(image.height  * (1 - ratio));
    }
