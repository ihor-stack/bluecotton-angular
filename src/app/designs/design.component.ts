import { Component, ElementRef, Input, OnInit, OnDestroy, ViewChild, TemplateRef } from '@angular/core';
...
export class DesignComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('quickBarMobileContent') quickBarMobileContent: TemplateRef<any>;
    ...
    this.designSubscription = this.designsService.events.subscribe((designEvent: DesignsEvent) => {
      switch (designEvent.type) {
        case 'CHANGE_DESIGN_PRODUCT': {
          ...
          this.designsService.isLoading = true;
          break;
        }
        case 'LOAD_DESIGN': {
          ...
          this.currentDesign = d;
          this.globalService.currentDesign = this.currentDesign;
          this.fontPickerService.loadFonts(this.currentDesign.fonts.map((f) => f.family)).subscribe(() => {
  ...
  onSelectColor(color: Color) {
    ...
    this.currentDesign.elements.forEach((e) => {
      e.colors().forEach((c) => {
        if (c.id === 'GARMENT_COLOR' && this.designsService.isLoading) {
          c.rgb = this.currentDesign.productColor.elementSwatch();
          e.render().subscribe();
        }
      });
    });
  ...
  showQuickMobileBar() {
    this.dialogService.open(this.quickBarMobileContent);
  }
  ...
  private colorFromProductColor(productColor: ProductColor): Color {
    ...
    if (productColor.maskRGBs.length > 0) {
      color.display = productColor.maskRGBs.map( color => {return { type: 'hex', value: `#${color.rgb}`}});
    } else {
