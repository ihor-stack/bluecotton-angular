...
export class Side {
  ...
  public colorOfImage: string = '';
  private productImageElement: any;
  private sideLabel: any;
  private sideNotes: any;
  private baseImageFilter = [241,241,241]; //constant for the Image with filter
    ...
    return Observable.create((observer) => {
      this.dataUriForImageUrl(sideURL).subscribe((maskImageDataUri) => {
        ...
        maskImage.onload = function() {
          ...
          const colorImageSpectr = context.getImageData( maskImage.width/2 - 50, maskImage.height/3 - 50 , 100, 100);
          self.setColorOfImage(colorImageSpectr.data);
  ...
  private setColorOfImage(canvasArray):void {
    let red = 0, green = 0, blue = 0;
    for ( let i=0; i < canvasArray.length; i+=4) {
      red += canvasArray[i];
      green += canvasArray[i+1];
      blue += canvasArray[i+2];
    }
    red = Math.floor(red / (canvasArray.length / 4));
    green = Math.floor(green / (canvasArray.length / 4));
    blue = Math.floor(blue / (canvasArray.length / 4));
    if (red + green + blue > 0) {
      this.colorOfImage = `#${this.rgbToHex(red)}${this.rgbToHex(green)}${this.rgbToHex(blue)}`;
    }
  }
  ...
  private multiply = (rgb1: Array<number>, rgb2: Array<number>):string => {
    const multiplyResultArray = rgb1.map((c, i) => this.rgbToHex(Math.floor(c * rgb2[i] / 255)));
    return multiplyResultArray.join('');
  }
        ...
        if (mask.imageUrl) {
          ...
          this.dataUriForImageUrl(mask.imageUrl).subscribe((maskImageDataUri) => {
            ...
            maskImage.onload = function() {
              ...
              if ( color.maskRGBs.length > 0) {
                self.colorOfImage = '#' + self.multiply(self.hexToRgb(color.maskRGBs[0].rgb), self.baseImageFilter);
              }
