...
export class Element {
  ...
  protected rebuildEnabledEffects(prop?: boolean) {
    this.effects().forEach((effect) => effect.buildEffect());
    this.setWrapperDimensions(prop);
  }

  setWrapperDimensions(flag?: boolean): void {
  ...
  subElementBox(outline: number = 0): SVG.Box {
    const subElementBox = new SVG.BBox();
    const box = this.accurateBoundingBox();
    subElementBox.x = box.x - outline;
    subElementBox.y = box.y - outline;
    subElementBox.w = box.width + outline * 2;
    subElementBox.h = box.height + outline * 2;
