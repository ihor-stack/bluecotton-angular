    ...
    return Observable.create((observer) => {
      this.getPlainTextCanvas().subscribe((textCanvas) => {
        ...
        this.text.split(/\n/).forEach((line, i) => {
          ...
          const dd = arch(w + (d - dim.d) / 2, h + 40, fontSize, this.font.family, line, angle, clockwise);
        ...
        const rateExpected = boxWidth / boxHeight;
        const rateCurrent = this.element.width / this.element.height;
        if (rateExpected.toFixed(2) !== rateCurrent.toFixed(2)) {
          // this.element.resize(boxWidth, boxHeight);
        }
        ...
        svgImg.onload = () => {
          ...
          // scaledContext.drawImage(svgImg, 0, 0, scaledCanvas.width, scaledCanvas.height);
