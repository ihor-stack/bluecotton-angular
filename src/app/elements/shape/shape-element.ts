...
export class ShapeElement extends Element {
  ...
  get outlined() { return this.outline}


  public render(prop?: boolean): Observable<void> {    
    return Observable.create((observer) => {
      ...
      if (this.fill.id === 'GARMENT_COLOR') {
        this.fill.rgb = this.area.design.productColor.elementSwatch();
      }
      ...
      if (this.x === undefined || this.y === undefined) {
        ...
        const rbox = this.subElementBox(this.outline);
      ...
      this.rebuildEnabledEffects(prop);

      this.scaleToFit();
      this.moveToFit();
      const box = this.subElementBox(this.outline);
        this.svgElement
          .x(this.x )
          .y(this.y )
          .width(this.width)
  ...
  rotate(degrees: number) {
    ...
    if (this.hasPoints) {
      ...
      const subElementBox = this.subElementBox(this.outline);
  ...
  get icon(): string {
    return '<i class="las la-shapes menu-icon"></i>';
  }
