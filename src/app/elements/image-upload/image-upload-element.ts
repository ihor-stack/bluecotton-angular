...
export class ImageUploadElement extends Element {
  ...
  static newFromJson(json: object, area: Area, image: Image, imagesService: ImagesService): ImageUploadElement {
    const element = new ImageUploadElement(area, image, imagesService);
  ...
  constructor(area: Area, image: Image, private imagesService: ImagesService) {
    super(area);
    this.image = image;
    this.threshold = 364;
    this.imageColors = [];
  ...
  set threshold(value: number) { 
    this.imagesService.getImage(this.image).subscribe( image => {
      this.image = image;
    });
  ...
  get icon(): string {
    return '<i class="las la-upload menu-icon"></i>';
  }
