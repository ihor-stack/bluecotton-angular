...
export class FontPickerService {
  ...
  private cachedFonts = {};
  private loadedFonts = [];
  private options = new RequestOptions({withCredentials: false });
  ...
  public getAvailableFonts(): Observable<FontFromApi[]> {
    ...
    return this.http.get(this.fontsUrl, this.options)
  ...
  */
  public listAllFonts(sort: string): Observable<Font[]> {
    ...
    return <Observable<Font[]>> this.http.get(requestUrl, this.options)
